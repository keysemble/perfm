...
from PyQt5.QtWidgets import QApplication, QMainWindow, QFileSystemModel, QMessageBox, \
                            QStatusBar, QDialog, QStyle, QAction
...
qt_creator_file = os.path.join(project_directory, 'ui', 'design_window.ui')
app_icon = os.path.join(project_directory, 'icons', 'Icon.ico')
...
class PerfmWindow(QMainWindow, DesignWindow):
    ...
    def open_file_information(self):
        ...
        self.file_size_value.setText(
            str(HumanBytes.format(qfile.size(), precision=2)))
        ...
        spinboxes=(self.user_octal_spin_box,
                   self.group_octal_spin_box, self.other_octal_spin_box)
